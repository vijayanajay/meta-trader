...
@pytest.fixture
def mock_download_with_retry() -> Iterator[MagicMock]:
    """Fixture to mock the internal _download_with_retry function."""
    with patch("self_improving_quant.services.data_service._download_with_retry") as mock:
...
def test_fetch_no_cache(
    mock_to_parquet: MagicMock, mock_download_with_retry: MagicMock, mock_cache_file: MagicMock
) -> None:
    ...
    mock_download_with_retry.assert_called_once()
...
def test_fetch_with_fresh_cache(
    mock_pd_read_parquet: MagicMock, mock_download_with_retry: MagicMock, mock_cache_file: MagicMock
) -> None:
...
def test_fetch_download_fails(mock_download_with_retry: MagicMock, mock_cache_file: MagicMock) -> None:
    """Test that an exception during download is propagated."""
    mock_cache_file.exists.return_value = False
    mock_download_with_retry.side_effect = ValueError("Download failed")

    with pytest.raises(ValueError, match="Download failed"):
        fetch_and_split_data(TICKER)
