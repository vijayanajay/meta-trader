...
-   `indicators`: A list of indicators to be calculated by the internal library. The `name` is the variable name that can be used in the conditions. `function` must be a function supported by the `StrategyEngine`.
