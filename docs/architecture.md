...
### Task 36 — Decouple `LLMAuditService`

*   **Rationale:** The LLM audit is an *offline analysis tool*, not part of the core signal generation path. It violates H-13 and H-25. It should be a separate script that can be run on the *output* of the main pipeline (`opportunities.md`).
*   **Items to implement:**
    1.  Remove `LLMAuditService` from the `Orchestrator`.
    2.  Create a new script `scripts/audit_opportunities.py` for offline auditing.
*   **Status:** Done
*   **Resolution:** The `LLMAuditService` has been completely decoupled from the `Orchestrator`. All LLM-related logic was removed from the core backtesting pipeline. A new skeleton script `scripts/audit_opportunities.py` has been created to house the offline audit functionality.

---

### Task 37 — Simplify `ConfigService`

*   **Rationale:** The `ConfigService` was implemented as a class, which is unnecessary boilerplate. A simple `load_config(path)` function is sufficient and reduces complexity.
*   **Items to implement:**
    1.  Refactor `ConfigService` class into a `load_config` function.
    2.  Update all call sites in `main.py` and tests to use the new function.
*   **Status:** Done
*   **Resolution:** The `ConfigService` class has been refactored into a stateless `load_config` function. All call sites and tests have been updated, and the test suite passes.

---

### Task 38 — Consolidate Core Logic

*   **Rationale:** The distinction between `core` and `services` was arbitrary. Essential pipeline components like `DataService` and `ExecutionSimulator` belong in `core`.
*   **Items to implement:**
    1.  Move `data_service.py` and `execution_simulator.py` from `services/` to `core/`.
    2.  Update all import statements that reference the moved files.
*   **Status:** Done
*   **Resolution:** The specified files have been moved to the `core` directory, and all import paths throughout the codebase have been updated to reflect the new locations.

---

### Task 39 — Add `py.typed` files

*   **Rationale:** Enforce type checking across the project by adding a `py.typed` file to every module directory, as a simple but powerful way to improve code quality.
*   **Items to implement:**
    1.  Create empty `py.typed` files in `praxis_engine/core/`, `praxis_engine/services/`, and `praxis_engine/core/guards/`.
*   **Status:** Done
*   **Resolution:** Empty `py.typed` files have been added to all required module subdirectories, satisfying the strict type checking requirement.
