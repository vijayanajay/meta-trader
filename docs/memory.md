...
1.  **Duplicate DataFrame Columns Causing Ambiguity:** Tests were failing with `ValueError: The truth value of a Series is ambiguous`. This was caused by having duplicate column names in a pandas DataFrame (e.g., two columns named `ATR_14`). When a column name is not unique, selecting it (`df['ATR_14']`) returns a DataFrame containing all columns with that name, not a Series. Subsequent operations that expect a scalar value (like `pd.isna()`) then receive a Series, leading to the ambiguity error.
    *   **Fix:** The root cause was that the test data was providing a hardcoded indicator column (`ATR_14`), while the `precompute_indicators` function, called during the backtest, was also calculating and adding a column with the same name. The fix was to remove the hardcoded indicator columns from the test data and let the `precompute_indicators` function be the single source of truth for their creation.
    *   **Lesson:** When writing integration tests for a data processing pipeline, be aware of all the steps that modify the DataFrame. Avoid providing pre-calculated columns in test data if a function down the line is supposed to calculate them. This ensures the test environment accurately reflects the real data flow and avoids subtle bugs from duplicate columns.

2.  **Brittle Test Assertions and Data:** Tests were failing with `AssertionError` because the expected outcomes (like exit dates or prices) did not match the actual results.
    *   **Issue:** The test data was not correctly set up to trigger the intended behavior. For example, the data to test an ATR stop-loss did not produce an ATR value that would actually cause the stop-loss to be hit at the asserted price. Similarly, a test for a profit-taking exit was asserting an exit on a day when the conditions for the exit were not yet met.
    *   **Fix:** The test data was carefully adjusted to create the precise conditions needed to trigger the specific logic being tested. Assertions were updated to reflect the correct, expected outcomes based on the new data and the code's logic. In some cases, this involved mocking the `precompute_indicators` function to provide exact indicator values for the test, making the test more controlled and less brittle.
    *   **Lesson:** Test data and assertions must be meticulously crafted. For complex calculations (like indicators), it is often better to mock the calculation's output to create a controlled environment, rather than trying to reverse-engineer input data that produces a desired output. This makes tests more robust and easier to understand.

## Task 30 & 35 Learnings: The Importance of Symmetrical Exits

1.  **A Falsified Hypothesis: The "Exit at the Mean" Flaw:** An experiment was conducted in **Task 30** to implement a profit-taking exit when the price reverted to the mean (the middle Bollinger Band). The hypothesis was that this would be a scientifically sound exit for a mean-reversion strategy.
    *   **Outcome:** The backtest **conclusively falsified** this hypothesis. The implementation was functionally correct, but the strategic outcome was a catastrophic failure.
    *   **Evidence:** The key evidence was the collapse of the average win from a profitable `11.62%` to a disastrous `3.19%`, while the average loss remained stable. This destroyed the strategy's risk/reward profile and rendered it unprofitable. Exiting at the mean systematically cut off the most profitable part of the reversion.
    *   **Lesson:** A strategy's exit logic must be philosophically and mathematically consistent with its entry signal's statistical premise. For mean reversion, this means allowing the reversion to complete its cycle. The entry is a bet on a reversion *from* an extreme (the lower band); the exit must therefore target the *opposite* extreme (the upper band) to capture the full move. This learning was implemented in **Task 35**.

---
### Epic 11 Testing Learnings: Isolating Multiprocessing in Tests

1.  **The Challenge of Testing `multiprocessing` with `pytest` and `patch`:** During the implementation of the CSV export (Task 43), a test for the `backtest` CLI command (`test_backtest_cli_creates_csv`) proved extremely brittle and difficult to fix. The test repeatedly failed with a variety of obscure errors (`NameError`, `TypeError`, `PicklingError`) related to the interaction between `pytest` fixtures, `unittest.mock.patch`, and the `multiprocessing` module used by the CLI.
    *   **Root Cause:** Patching objects in a main test process does not reliably carry over to child processes spawned by `multiprocessing.Pool`. This can lead to child processes using real objects instead of mocks, or failing to pickle the mock objects themselves.
    *   **Pragmatic Solution:** Instead of trying to debug the complex interactions between these libraries, the most robust and pragmatic solution was to avoid multiprocessing in the test altogether. A custom `MockPool` class was created that mimics the `multiprocessing.Pool` interface but executes the code serially in the main thread using a simple `map` function.
    *   **Lesson:** When testing code that uses `multiprocessing`, it is often better to isolate the logic being tested from the parallelism itself. If the goal is to test the aggregation of results from workers, and not the performance of the parallel execution, mocking the `Pool` to run serially is a clean, fast, and reliable pattern. This avoids brittle tests and allows the focus to remain on the correctness of the application logic.

## Epic 13 Learnings: The Failed Regime Meta-Model

1.  **A Falsified Hypothesis: Multi-Factor Model vs. Simple Heuristic:** Epic 13 was designed to replace the simple, single-factor `RegimeGuard` (based on sector volatility) with a more sophisticated, multi-factor logistic regression model. The hypothesis was that a model trained on market-wide features (Nifty's position vs. its 200-day MA, VIX level, VIX rate-of-change) would be better at identifying high-risk, trending regimes and would therefore reduce the strategy's catastrophic drawdowns.
    *   **Outcome:** The backtest **conclusively falsified** this hypothesis. The implementation of the automated training and integration was successful, but the model's performance was a failure.
    *   **Evidence:** The new backtest report was compared against the baseline.
        *   **Maximum Drawdown:** The primary success metric, showed no improvement. (Baseline: **-67.03%**, New Model: **-67.78%**).
        *   **Overall Returns:** Key performance metrics were significantly worse. Net Annualized Return dropped from `160.39%` to `76.72%`, and the Sharpe Ratio fell from `1.64` to `1.16`.
        *   **2020 Crash:** The model completely failed to provide protection during the critical 2020 COVID-19 crash, with the "Maximum Drawdown Analysis" section showing a drawdown of over 95% during that specific period.
    *   **Root Cause Analysis:** The model successfully filtered out a large number of trades (from 530 to 335), but it did so indiscriminately. It appears the features chosen, while plausible, were not effective at differentiating between "good" and "bad" regimes for this specific mean-reversion strategy. The model threw the baby out with the bathwater, reducing overall returns without mitigating the primary risk.
    *   **Lesson:** Complexity is not a guaranteed path to improvement. A simple, understandable heuristic (like "don't trade when sector volatility is high") can sometimes outperform a more complex, opaque model if the model's features are not correctly specified or do not have a strong predictive relationship with the target variable. This experiment serves as a strong reminder to rigorously validate any new model against a simple baseline before fully integrating it.

## Backtest Data Alignment Learnings

1.  **Temporal Misalignment of Market Features:** The backtester was producing warnings like `Missing one or more market feature columns`. This was caused by a mismatch in trading days between individual stocks and the market indices (Nifty, VIX) used for calculating regime features. On days when a stock traded but the overall market was closed (e.g., certain holidays), the `left join` in the `Orchestrator` would correctly produce `NaN` values for the market feature columns. This caused downstream components like the `RegimeGuard` to fail or fall back to default behavior.
    *   **Fix:** A forward-fill (`ffill()`) was applied to the market feature columns immediately after the join in `Orchestrator.run_backtest`. This propagates the last known market regime data into the gaps, ensuring that every trading day for a stock has a valid set of market features. This is a common and robust practice for handling asynchronous time-series data.
    *   **Lesson:** When combining time-series data from different sources (e.g., individual stocks and market indices), never assume their calendars are perfectly aligned. Always anticipate and handle `NaN` values that arise from joins. For state-based data like market regimes, forward-filling is a logically sound approach to ensure continuity.
