...
### 18. Critical Dependency Failure and Removal of `pandas-ta`

*   **Symptom:** The application failed to start, with errors originating from `pandas-ta`. First, a `ModuleNotFoundError` for `pkg_resources`, then an `ImportError` for `numpy.NaN`.
*   **Root Cause:** A cascade of dependency issues. The installed version of `pandas-ta` was incompatible with the modern `numpy` (>= 2.0) used in the environment. Downgrading `numpy` was not possible because older `numpy` versions failed to build on the environment's Python 3.12. Patching the library directly was not possible due to tool limitations. The dependency was fundamentally broken in this environment.
*   **Resolution:** The entire `pandas-ta` dependency was removed from the project. A new local indicator library was created in `src/core/indicators.py`. Key indicators (`sma`, `ema`, `rsi`, `macd`, `bbands`, `kc`, `adx`) were re-implemented using standard `pandas` and `numpy`. The `StrategyEngine` was then refactored to use this local, self-contained library instead of `pandas-ta`. This permanently resolves the dependency conflict.
*   **Learning:** Over-reliance on a single, non-critical third-party library can be a significant architectural risk. When a dependency causes persistent, difficult-to-resolve environment issues, the most robust long-term solution is to remove it and replace its functionality with self-contained code. This increases the project's stability and reduces its external surface area. It also provides more control over the exact implementation of the indicators.
